...
export class channelUpdateEvent extends Event {
  execute(oldChannel: Channel, newChannel: Channel): void {
    ...
    if (oldGuildChannel.name !== newGuildChannel.name) {
      this.bot.logger.admin_blue(`Der Kanal \`${oldGuildChannel.name}\` heißt jetzt \`${newGuildChannel.name}\``);
    }

    if (oldGuildChannel.parent !== newGuildChannel.parent) {
      this.bot.logger.admin_blue(
